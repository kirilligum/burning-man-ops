...
you do not know an answer, identify the responsible camp role rather than
guessing.

Do not put personal contact details, payment information, credentials, or
private camper data in this file. Those belong in the runtime materials held
by the Dominatrix.

## Priority 0 — decide before the related activity is allowed

### 1. Which unresolved activities will actually operate in 2026?
...
### 3. What is the approved 2026 flame-effect procedure?

If flame effects will operate, provide the current FAST-approved procedure or
have the Flame Lead supply:

- the physical FAST-license status;
- photographs and exact IDs for every valve and control;
- approved pressure, temperature, voltage, and timing limits;
- startup, test, normal-shutdown, and emergency-shutdown sequences;
- wind limits, exclusion zone, and operator requirements.

Until this exists, the flame task cards are work-package boundaries and do not
authorize operation.
...
lighting, shower, sink, and sound installation. At minimum, confirm actual part
counts, anchor type and placement, crew size, lift/lower order, wind stop
conditions, securement method, and final-state photographs.

Which document is authoritative, and which structures still lack these facts?
...
- generator model and fuel type;
- whether campers ever refuel it or only an approved provider does;
- the authorized operator or provider method;
- the shutdown, cooling, spill-control, and restart method from its manual.

This answer controls `Read_generator_fuel_level` and
`Coordinate_generator_refueling`.
...
cylinder. Confirm whether the provisional replacement-cylinder description is
correct or should be replaced.
...
generators, fuel/propane, dumpster, emergency equipment, storage, and task-card
stations. Exclude private sleeping assignments and vehicle ownership.

Where will the current map be available on playa?
...
details. Who prepares it, who keeps it current, and how does each Dominatrix
access it?

Only stable role names belong in repository XML.
...
Recycle Camp. These answers should replace avoidable “ask the Dominatrix”
fallbacks.
...
Lost and Found container should be placed when the first is full.
...
trash bags, drinking-water hose, propane leak-test solution, gloves, and other
consumables. Photographs of the product and label are sufficient; do not send
...
`ESCALATED` results into the camp issue log, and where completed records are
kept.
